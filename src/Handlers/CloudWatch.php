...
class CloudWatch extends AbstractProcessingHandler
{
    ...
    {
        if(isset($entry['context']) && !isset($entry['context']['exception']) && is_array($entry['context']) && $entry['context'] !== []) {
            foreach($entry['context'] as $k => $v) {
                if(str_contains($entry['message'], "{" . $k . "}"))
                  $entry['message'] = str_replace("{" . $k . "}", json_encode($v), $entry['message']);
            }
        }

        $entries = str_split($entry['message'], self::EVENT_SIZE_LIMIT);
