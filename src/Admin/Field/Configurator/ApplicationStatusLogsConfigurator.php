 ...
 */
final class ApplicationStatusLogsConfigurator implements FieldConfiguratorInterface
{
    ...
    {
        return ApplicationStatusLogField::class === $field->getFieldFqcn();
    }
    ...
    public function configure(FieldDto $field, EntityDto $entityDto, AdminContext $context): void
    {
        ...
        $targetCrudControllerFqcn = $field->getCustomOption(ApplicationStatusLogField::OPTION_CRUD_CONTROLLER)
            ?? $context->getCrudControllers()->findCrudFqcnByEntityFqcn($targetEntityFqcn);
        $field->setCustomOption(ApplicationStatusLogField::OPTION_CRUD_CONTROLLER, $targetCrudControllerFqcn);

        if (ApplicationStatusLogField::WIDGET_AUTOCOMPLETE === $field->getCustomOption(ApplicationStatusLogField::OPTION_WIDGET)) {
        ...
        if (true === $field->getCustomOption(ApplicationStatusLogField::OPTION_AUTOCOMPLETE)) {
            $targetCrudControllerFqcn = $field->getCustomOption(ApplicationStatusLogField::OPTION_CRUD_CONTROLLER);
            ...
            $autocompleteEndpointUrl = $this->adminUrlGenerator
                ...
                ->setController($field->getCustomOption(ApplicationStatusLogField::OPTION_CRUD_CONTROLLER))
                ->setAction('autocomplete')
                ->setEntityId(null)
                ->unset(EA::SORT) // Avoid passing the 'sort' param from the current entity to the autocompleted one
                ->set(ApplicationStatusLogField::PARAM_AUTOCOMPLETE_CONTEXT, [
            ...
            $field->setFormTypeOptionIfNotSet('query_builder', static function (EntityRepository $repository) use ($field) {
                ...
                if ($queryBuilderCallable = $field->getCustomOption(ApplicationStatusLogField::OPTION_QUERY_BUILDER_CALLABLE)) {
    ...
    {
        $field->setCustomOption(ApplicationStatusLogField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toOne');
        ...
        $field->setCustomOption(ApplicationStatusLogField::OPTION_RELATED_URL, $this->generateLinkToAssociatedEntity($targetCrudControllerFqcn, $targetEntityDto));
    ...
    {
        $field->setCustomOption(ApplicationStatusLogField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toMany');
